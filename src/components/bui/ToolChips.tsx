 ...
 *
 * Props: rows/diffs replace the demo data (kept as
 * defaults); `auto` drives the staggered entrance, so real
 * data can render all rows at once with auto={false}.
 * ───────────────────────────────────────────────────────── */
...
function StatusDot({ status }: { status?: ToolChipRow["status"] }) {
  ...
  if (status === "pending") {
    return (
      <span
        role="img"
        aria-label="运行中"
        className="size-2.5 shrink-0 rounded-full border-[1.5px] border-line-strong border-t-ink-2"
  ...
  return (
    <span
      role="img"
      aria-label={status === "error" ? "失败" : "已拒绝"}
      className={`size-1.5 shrink-0 rounded-full ${status === "error" ? "bg-red" : "bg-orange"}`}
    />
  );
}
...
export default function ToolChips({
  ...
  const [step, setStep] = useState(auto ? 0 : total);
  ...
  const visibleDiffs = diffs.slice(0, 3);
  const extraDiffs = diffs.length - visibleDiffs.length;
  ...
  return (
    <div className="min-h-[220px] w-full max-w-80 pb-1">
      ...
      >
        ...
        <span className="tabular-nums">{summaryLabel ?? `${rows.length} tool calls`}</span>
      </button>
        ...
        <div className="mt-2.5 flex max-w-full flex-wrap gap-1.5 border-t border-line pt-2.5">
          ...
          {extraDiffs > 0 && (
            <button
              ...
              style={{ animation: `fade-in 300ms ease-out ${visibleDiffs.length * 80}ms both` }}
            >
              +{extraDiffs} more
            </button>
          )}

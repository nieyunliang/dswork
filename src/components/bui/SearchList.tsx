...
export default function SearchList({ items = ITEMS, placeholder = "Search flavors…", maxResults, onSelect }: { items?: string[]; placeholder?: string; maxResults?: number; onSelect?: (item: string) => void } = {}) {
  const [query, setQuery] = useState("");
  const results = query
    ? items.filter((i) => i.toLowerCase().includes(query.toLowerCase()))
    : items;
  const visible = maxResults !== undefined ? results.slice(0, maxResults) : results;

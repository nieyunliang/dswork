...
export default function RecordsTable({ rows = INITIAL_ROWS, onRowClick }: { rows?: Row[]; onRowClick?: (row: Row) => void } = {}) {
        ...
        <table className="records-table">
          ...
          <tbody>
            {visibleRows.map((row) => {
              ...
              >
                <td className="records-cell records-sticky-cell records-company-cell"><Checkbox checked={selectedRow} onChange={() => toggleRow(row.id)} label={`Select ${row.name}`} /><span className="records-company-mark">{row.name.slice(0, 1).toUpperCase()}</span>{row.website ? <a href={`https://${row.website}`} className="records-company-name has-link">{row.name}</a> : <span className="records-company-name">{row.name}</span>}</td>

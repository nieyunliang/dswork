...
const ACTION_ICONS: { label: string; icon: React.ReactNode }[] = [
  { label: "复制", icon: <g><rect x="9" y="9" width="12" height="12" rx="2.5" /><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" /></g> },
  { label: "重新生成", icon: <path d="M21 12a9 9 0 1 1-2.64-6.36M21 3v6h-6" /> },
  { label: "上一个", icon: <path d="M7 10v12M15 5.88L14 10h5.83a2 2 0 0 1 1.92 2.56l-2.33 8A2 2 0 0 1 17.5 22H4a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h2.76a2 2 0 0 0 1.79-1.11L12 2a3.13 3.13 0 0 1 3 3.88z" /> },
  { label: "下一个", icon: <path d="M17 14V2M9 18.12L10 14H4.17a2 2 0 0 1-1.92-2.56l2.33-8A2 2 0 0 1 6.5 2H20a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2h-2.76a2 2 0 0 0-1.79 1.11L12 22a3.13 3.13 0 0 1-3-3.88z" /> },
];
...
export default function StreamingText({ text, tokens, sources = SOURCES, followUps = FOLLOW_UPS, loop = true, wordMs = WORD_MS, onFollowUp }: StreamingTextProps = {}) {
  const resolvedTokens = tokens ?? (text !== undefined ? text.split(" ").map((t) => ({ text: t } as Token)) : TOKENS);
  const [count, setCount] = useState(0);
  const [sourcesOpen, setSourcesOpen] = useState(false);
  const done = count >= resolvedTokens.length;

  useEffect(() => {
    if (done && !loop) return;
    const t = setTimeout(
      () => setCount((c) => (c >= resolvedTokens.length ? 0 : c + 1)),
      done ? HOLD_MS : wordMs,
    );
    return () => clearTimeout(t);
  }, [count, done, loop, wordMs, resolvedTokens.length]);

  return (
    <div className="min-h-[15.5rem] w-full max-w-95">
      <p aria-live="polite" className="text-[13px] leading-relaxed text-ink">
        {resolvedTokens.slice(0, count).map((token, i) =>
          token.cite ? (

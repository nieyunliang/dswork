...
function useStage(steps: number[], enabled: boolean) {
...
export default function DiffTable({
  ...
  title?: string;
  rows?: { id: string; dept: string; email: string; removed: boolean }[];
  animate?: boolean;
}) {
  const stage = useStage([800, 1000, 1000], animate);
  // 0 plain · 1 red tint · 2 completed diff
  const tinted = animate ? stage >= 2 : true;
  const added = animate ? stage >= 3 : true;
